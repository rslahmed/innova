...
if ($single_status['status']==1){
    $deactive = "UPDATE services SET status='0' WHERE id='$id'";
    $deactive_con = mysqli_query($db, $deactive);
    $_SESSION['active_msg'] = "Services has been <strong> Deactived! <strong>";
}elseif ($single_status['status']==0){
    $active = "UPDATE services SET status='1' WHERE id='$id'";
    $active_con = mysqli_query($db, $active);
    $_SESSION['active_msg'] = "Services has been <strong> actived! <strong>";
}
